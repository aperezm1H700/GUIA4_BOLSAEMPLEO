...
public class BolsaDeEmpleo {
        ...
        for(int i = 0; i < n; i++) {
        	for(int j = 0; j < n -1; j++) {
        		if(aspirantes.get(j).darNombre().compareTo(aspirantes.get(j+1).darNombre())>0) {
        			tem=aspirantes.get(j);
        			aspirantes.set(j, aspirantes.get(j+1));
        			aspirantes.set(j+1, tem);
        		}
        	}
        }
    }
    ...
    public void ordenarPorEdad() {
        ...
        for(int i=0; i<n-1;i++) {
        	...
        	pos=i;
        	for(int j=i+1;j<n;j++) {
        		if(aspirantes.get(j).darEdad()<menor.darEdad()) {
        			menor=aspirantes.get(j);
        			pos=j;
        		}
        	}
        	if(pos!=i) {
        		tem=aspirantes.get(i);
        		aspirantes.set(i,aspirantes.get(pos));
        		aspirantes.set(pos, tem);
        	}
        }
    }
    ...
    public void ordenarPorAniosDeExperiencia() {
        ...
        for(int i = 1;i < n;i++) {
	    	   int j = i;
	    	   while(j > 0 && aspirantes.get(j).darAniosExperiencia() < aspirantes.get(j - 1).darAniosExperiencia()) {
	    		   tem = aspirantes.get(j);
	    		   aspirantes.set(j, aspirantes.get(j-1));
	    		   aspirantes.set(j - 1, tem);
	    		   j--;
    ...
    public int buscarBinarioPorNombre(String nombre) {
        ...
        ordenarPorNombre();
        while(ini <= fin) {
            int middleIndex = (ini + fin) / 2;
            if (aspirantes.get(middleIndex).darNombre().compareTo(nombre)==0) {
                return middleIndex;
            }
            else if (aspirantes.get(middleIndex).darNombre().compareTo(nombre)<0)
                ini = middleIndex + 1;
            else if (aspirantes.get(middleIndex).darNombre().compareTo(nombre)>0)
                fin = middleIndex - 1;
    ...
    public int buscarAspiranteMasJoven() {
        ...
        if(aspirantes.size()>0) {
        	int menor = aspirantes.get(0).darEdad();
        	posicion = 0;
    ...
    public int buscarAspiranteMayorEdad() {
        ...
        if(aspirantes.size()>0) {
        	int mayor = aspirantes.get(0).darEdad();
        	posicion = 0;
        	for(int i=1; i < aspirantes.size(); i++) {
    ...
    public int buscarAspiranteMayorExperiencia() {
        ...
        if(aspirantes.size()>0) {
        	int mayor = aspirantes.get(0).darAniosExperiencia();
        	posicion = 0;
        	for(int i=1; i < aspirantes.size(); i++) {
        		if(aspirantes.get(i).darAniosExperiencia() > mayor) {
        			mayor=aspirantes.get(i).darAniosExperiencia();
        			posicion=i;
        		}
        	}
        }
    ...
    public int eliminarAspirantesPorExperiencia(int aniosExperiencia) {
        ...
        int cont = 0;
       	...
       	do {
       		if(aspirantes.get(cont).darAniosExperiencia()==aniosExperiencia) {
